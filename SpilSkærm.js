class SpilSkærm {
  ...
  run() {
    ...
    text("SORT: " + this.tid2, width/2, height/1.6);
        ...
        if (this.spiller1True) {
  if (this.tid1 <= 0) {
    manager.opretGameState('slut',new AfslutSkærm('Sort'))
    manager.skiftGameState("slut", new AfslutSkærm("Sort"));
    return;
  }
} else {
  if (this.tid2 <= 0) {
    manager.opretGameState('slut', new AfslutSkærm('Hvid'))
    manager.skiftGameState("slut", new AfslutSkærm("Hvid"));
    return;
  }
}
